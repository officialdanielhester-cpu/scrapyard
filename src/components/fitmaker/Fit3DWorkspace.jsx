...
function objectFromSpec(spec) {
  ...
  if (spec.kind === "image" || spec.imageUrl) {
    return { id: newId(), name: spec.name || "Model", kind: "image", imageUrl: spec.imageUrl, pos: [0, 1, 0], scale: spec.scale || 1, rot: [0, 0, 0], metal: 0.3, rough: 0.7, isGarment: !!spec.isGarment };
  }
  let parts;
...
export default function Fit3DWorkspace({ garmentImage, garmentName }) {
  ...
  const pushUndo = useCallback(() => { setUndoStack((s) => [...s.slice(-29), snapshot(objectsRef.current)]); setRedoStack([]); }, [snapshot]);
  const undo = useCallback(() => { setUndoStack((s) => { if (!s.length) return s; const prev = s[s.length - 1]; setRedoStack((r) => [...r, snapshot(objectsRef.current)]); setObjects(prev); setSelectedId(null); setSelectedFaces(new Set()); return s.slice(0, -1); }); }, [snapshot]);
  const redo = useCallback(() => { setRedoStack((r) => { if (!r.length) return r; const next = r[r.length - 1]; setUndoStack((s) => [...s, snapshot(objectsRef.current)]); setObjects(next); setSelectedId(null); setSelectedFaces(new Set()); return r.slice(0, -1); }); }, [snapshot]);

  const addMesh = (type) => { pushUndo(); const o = { id: newId(), name: type.charAt(0).toUpperCase() + type.slice(1), kind: "mesh", meshType: type, geo: createPrimitive(type), pos: [0, 0.8, 0], rot: [0, 0, 0], scale: 1, metal: 0.3, rough: 0.55 }; setObjects((p) => [...p, o]); setSelectedId(o.id); setSelectedFaces(new Set()); };
  ...
  // Seed / update the garment model from the 2D design render.
  useEffect(() => {
    if (!garmentImage) return;
    setObjects((prev) => {
      const garment = prev.find((o) => o.isGarment);
      if (garment) return prev.map((o) => o.isGarment ? { ...o, imageUrl: garmentImage, name: garmentName || o.name } : o);
      const o = objectFromSpec({ kind: "image", imageUrl: garmentImage, name: garmentName || "Garment", scale: 1.4, isGarment: true });
      setSelectedId(o.id);
      return [...prev, o];
    });
  }, [garmentImage, garmentName]);
    ...
    <div className="flex h-full min-h-0 flex-col gap-4 lg:flex-row">
      <div className="flex-1">
        ...
        <p className="mt-2 font-mono text-[10px] uppercase tracking-wider text-muted-foreground/60 md:hidden">
          touch drag empty to orbit · drag object to move · pinch to zoom · two-finger pan
        </p>
      </div>

      <div className="hidden lg:block">
        <StudioSidebar
          objects={objects} selectedId={selectedId} mode={mode} brush={brush} brushSize={brushSize}
          brushStrength={brushStrength} paintColor={paintColor} theme={theme} snap={snap} cameraLock={cameraLock}
          canUndo={undoStack.length > 0} canRedo={redoStack.length > 0}
          onModeChange={setMode} onBrushChange={setBrush} onBrushSize={setBrushSize} onBrushStrength={setBrushStrength}
          onPaintColor={setPaintColor} onToggleTheme={() => setTheme(t => t === "dark" ? "light" : "dark")}
          onToggleSnap={() => setSnap(s => !s)} onToggleCameraLock={() => setCameraLock(v => !v)}
          onUndo={undo} onRedo={redo} onSelectObject={setSelectedId} onUpdateObject={updateObject}
          onDelete={handleDelete} onDuplicate={handleDuplicate} onAddMesh={addMesh}
          onExtrude={handleExtrude} onSubdivide={handleSubdivide} onMerge={handleMerge} onMirror={handleMirror} onDeleteFaces={handleDeleteFaces}
          onImport={() => setImportOpen(true)} onGenerate={() => setJabberOpen(true)} onPresets={() => setPresetOpen(true)}
        />
      </div>
      ...
      <Sheet>
        ...
        <SheetContent side="right" className="w-80 overflow-y-auto p-0">
          <SheetHeader className="px-4 pt-4"><SheetTitle className="font-mono text-xs uppercase tracking-wider">Studio Panel</SheetTitle></SheetHeader>
          <div className="p-2">
            <StudioSidebar
              objects={objects} selectedId={selectedId} mode={mode} brush={brush} brushSize={brushSize}
              brushStrength={brushStrength} paintColor={paintColor} theme={theme} snap={snap} cameraLock={cameraLock}
