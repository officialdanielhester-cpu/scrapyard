...
export default function StudioSidebar(props) {
  ...
  const btn = "flex items-center gap-1.5 rounded-md border border-border/60 px-2.5 py-1.5 text-xs transition-colors hover:border-primary hover:text-primary disabled:opacity-40";
  const toolBtn = (active) => `flex items-center gap-1.5 rounded-md border px-2.5 py-1.5 text-xs transition-colors ${active ? "border-primary bg-primary/10 text-primary" : "border-border/60 hover:border-primary hover:text-primary"}`;
    ...
    <div className="flex w-full flex-col gap-3 lg:w-72">
      ...
      <div className="flex gap-1 rounded-xl border border-border/50 p-1">
        {MODES.map((m) => { const Icon = m.icon; return (
          <button key={m.id} onClick={() => props.onModeChange(m.id)} className={`flex flex-1 flex-col items-center gap-1 rounded-lg py-2 transition-colors ${mode === m.id ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}>
            <Icon className="h-4 w-4" strokeWidth={1.5} /><span className="font-mono text-[9px] uppercase tracking-wider">{m.label}</span>
          </button>); })}
      </div>
      ...
      {mode === "object" && (
        <div className="space-y-2 rounded-2xl border border-border/50 p-3">
          ...
          <div className="relative">
            <button onClick={() => setAddOpen(o => !o)} className={`flex w-full items-center gap-1.5 ${btn}`}><Plus className="h-3.5 w-3.5" /> Add Mesh <ChevronDown className="ml-auto h-3 w-3" /></button>
            {addOpen && (
              <div className="mt-1 grid grid-cols-3 gap-1 rounded-lg border border-border/60 bg-background p-1">
          ...
          <div className="flex gap-1">
            <button onClick={props.onImport} className={btn}><Download className="h-3.5 w-3.5" /> Import</button>
            <button onClick={props.onGenerate} className={btn}><Sparkles className="h-3.5 w-3.5" /> Generate</button>
            <button onClick={props.onPresets} className={btn}><Boxes className="h-3.5 w-3.5" /> Models</button>
          </div>
          <div className="flex gap-1">
            <button onClick={props.onDuplicate} disabled={!sel} className={btn}><Copy className="h-3.5 w-3.5" /> Duplicate</button>
            <button onClick={props.onDelete} disabled={!sel} className={`${btn} text-destructive hover:border-destructive`}><Trash2 className="h-3.5 w-3.5" /> Delete</button>
          </div>
          <label className="flex items-center gap-2 pt-1"><input type="checkbox" checked={snap} onChange={(e) => props.onToggleSnap()} className="accent-primary" /><span className="text-xs text-muted-foreground">Snap to grid</span></label>
        </div>
      )}
        ...
        <div className="space-y-2 rounded-2xl border border-border/50 p-3">
          ...
          {!isMesh ? <p className="text-xs text-muted-foreground">Select a mesh object to edit</p> : (
            <>
              ...
              <div className="flex gap-1">
                <button onClick={props.onMerge} className={`${toolBtn(false)} flex-1`}><GitMerge className="h-3.5 w-3.5" /> Merge</button>
                <button onClick={props.onMirror} className={`${toolBtn(false)} flex-1`}><FlipHorizontal className="h-3.5 w-3.5" /> Mirror</button>
              </div>
              <button onClick={props.onDeleteFaces} className={`${toolBtn(false)} w-full text-destructive hover:border-destructive`}><Trash2 className="h-3.5 w-3.5" /> Delete Faces</button>
            </>
          )}
        </div>
      )}
      ...
      {mode === "paint" && (
        <div className="space-y-3 rounded-2xl border border-border/50 p-3">
          ...
          {!isMesh ? <p className="text-xs text-muted-foreground">Select a mesh object to paint</p> : (
            <>
              <div><label className="mb-1 block font-mono text-[10px] uppercase text-muted-foreground">Color</label><input type="color" value={paintColor} onChange={(e) => props.onPaintColor(e.target.value)} className="h-8 w-full cursor-pointer rounded border border-border/60 bg-transparent" /></div>
              <div><label className="mb-1 block font-mono text-[10px] uppercase text-muted-foreground">Brush Size {brushSize.toFixed(2)}</label><input type="range" min="0.1" max="3" step="0.05" value={brushSize} onChange={(e) => props.onBrushSize(Number(e.target.value))} className="w-full accent-primary" /></div>
              <div><label className="mb-1 block font-mono text-[10px] uppercase text-muted-foreground">Strength {Math.round(brushStrength * 100)}%</label><input type="range" min="0.05" max="1" step="0.05" value={brushStrength} onChange={(e) => props.onBrushStrength(Number(e.target.value))} className="w-full accent-primary" /></div>
            </>
          )}
      ...
      <div className="rounded-2xl border border-border/50 p-3">
        <div className="mb-2 flex items-center gap-2">
          ...
          <div className="ml-auto flex items-center gap-1 rounded-md border border-border/40 px-1.5"><Search className="h-3 w-3 text-muted-foreground" /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search…" className="w-20 bg-transparent py-0.5 text-xs focus:outline-none" /></div>
        </div>
