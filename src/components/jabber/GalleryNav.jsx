...
import { Images, Folder, ChevronDown } from "lucide-react";

// Placeholder Gallery section for the sidebar — two collapsible dropdown menus
// with sample items. Self-contained: manages its own open state.
const GROUPS = [
  { id: "col-a", label: "Collection One", items: ["Item 1", "Item 2", "Item 3"] },
  { id: "col-b", label: "Collection Two", items: ["Item 1", "Item 2", "Item 3"] },
];

export default function GalleryNav({ variant = "desktop" }) {
              ...
              <div key={g.id} className="px-1 py-0.5">
                ...
                {subOpen === g.id && (
                  <ul className="my-1 ml-3 flex flex-col gap-0.5 border-l border-border/30 pl-2">
                    {g.items.map((label, i) => (
                      <li key={i} className="rounded-md px-2 py-1 text-xs text-muted-foreground">
                        {label}
                      </li>
                    ))}
                  </ul>
